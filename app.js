...
// ---------- Google Routes API：交通時間／大眾運輸最快路線 ----------
const GOOGLE_ROUTES_ENDPOINT = 'https://routes.googleapis.com/directions/v2:computeRoutes';
const GOOGLE_PLACES_ENDPOINT = 'https://places.googleapis.com/v1/places:searchText';
const GOOGLE_PLACES_CACHE_STORAGE = 'google_places_id_cache_v1';
const GOOGLE_PLACES_CACHE_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const GOOGLE_ROUTES_CACHE_STORAGE = 'google_routes_result_cache_v1';
const GOOGLE_ROUTES_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000; // 個人使用：快取 7 天
const googleRoutesInFlight = new Map(); // 防止同一筆路線在短時間內重複送出 API 請求

// API Key 不再由使用者輸入，也不寫入 localStorage。
// App 會自動載入同目錄的 config.js；你只需要在自己的電腦設定一次。
let googleConfigPromise = null;
async function loadGoogleConfig() {
  if (window.GOOGLE_ROUTES_API_KEY || window.APP_CONFIG?.GOOGLE_ROUTES_API_KEY) return;
  if (!googleConfigPromise) {
    googleConfigPromise = import('./config.js?cache=' + Date.now()).catch(() => null);
  }
  await googleConfigPromise;
}

async function getGoogleRoutesApiKey() {
  await loadGoogleConfig();
  return String(window.GOOGLE_ROUTES_API_KEY || window.APP_CONFIG?.GOOGLE_ROUTES_API_KEY || '').trim();
}

function changeGoogleRoutesApiKey() {
  alert('API Key 已改由 config.js 管理，不需要在 App 內輸入。請在 config.js 設定後重新整理頁面。');
}

function extractDateFromDayName(dayName) {
  const m = String(dayName || '').match(/(20\d{2})\s*[\/\-年]\s*(\d{1,2})\s*[\/\-月]\s*(\d{1,2})/);
  if (!m) return null;
  return { year: Number(m[1]), month: Number(m[2]), day: Number(m[3]) };
}

function buildDepartureTime(dayName, spotTime) {
  const d = extractDateFromDayName(dayName);
  const tm = String(spotTime || '').match(/^(\d{1,2}):(\d{2})$/);
  if (!d || !tm) return null;
  // Routes API Transit 要求 RFC3339 UTC (Zulu) 時間。日本時間 +09:00 轉成 UTC。
  const local = new Date(`${d.year}-${String(d.month).padStart(2, '0')}-${String(d.day).padStart(2, '0')}T${String(Number(tm[1])).padStart(2, '0')}:${tm[2]}:00+09:00`);
...
function readGooglePlaceIdCache(query) {
  try {
    const cache = JSON.parse(localStorage.getItem(GOOGLE_PLACES_CACHE_STORAGE) || '{}');
    const key = normalizeRouteCachePart(query);
    const entry = cache[key];
    if (!entry || !entry.savedAt || !entry.placeId) return null;
    if (Date.now() - Number(entry.savedAt) > GOOGLE_PLACES_CACHE_TTL_MS) {
      delete cache[key];
      localStorage.setItem(GOOGLE_PLACES_CACHE_STORAGE, JSON.stringify(cache));
      return null;
    }
...
function writeGooglePlaceIdCache(query, placeId) {
  try {
    const cache = JSON.parse(localStorage.getItem(GOOGLE_PLACES_CACHE_STORAGE) || '{}');
    cache[normalizeRouteCachePart(query)] = { savedAt: Date.now(), placeId };
    const entries = Object.entries(cache).sort((a,b) => Number(b[1]?.savedAt||0)-Number(a[1]?.savedAt||0)).slice(0, 200);
    localStorage.setItem(GOOGLE_PLACES_CACHE_STORAGE, JSON.stringify(Object.fromEntries(entries)));
  } catch (_) {}
}

async function resolvePlaceId(apiKey, placeName) {
  const base = normalizePlaceQuery(placeName);
  if (!base) return null;
  const query = /日本|Japan|大阪|京都|奈良|東京|神戸|兵庫|滋賀|和歌山/.test(base) ? base : `${base}, Japan`;
  const cached = readGooglePlaceIdCache(query);
  ...
  const response = await fetch(GOOGLE_PLACES_ENDPOINT, {
    ...
    body: JSON.stringify({ textQuery: query, languageCode: 'zh-TW', regionCode: 'JP', pageSize: 1 })
  });
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Places API ${response.status}: ${text.slice(0, 220)}`);
  }
  const data = await response.json();
  const placeId = data?.places?.[0]?.id || null;
  if (placeId) writeGooglePlaceIdCache(query, placeId);
  return placeId;
}
...
function formatTransitFare(fare) {
  if (!fare) return '票價資訊未提供';
  const units = Number(fare.units || 0);
  const nanos = Number(fare.nanos || 0);
  const amount = units + nanos / 1e9;
  if (!Number.isFinite(amount)) return '票價資訊未提供';
  return `${fare.currencyCode || ''} ${amount.toFixed(amount % 1 ? 2 : 0)}`.trim();
}

function getTransitStepSummary(step) {
  const td = step?.transitDetails;
  if (!td) return null;
  const line = td.transitLine || {};
  const vehicle = line.vehicle || {};
  const start = td.stopDetails?.departureStop?.name?.text || td.stopDetails?.departureStop?.name || '';
  const end = td.stopDetails?.arrivalStop?.name?.text || td.stopDetails?.arrivalStop?.name || '';
  const lineName = line.nameShort || line.name || vehicle.name?.text || vehicle.name || '';
  const headsign = td.headsign || '';
  return { start, end, lineName, headsign, vehicle: vehicle.name?.text || vehicle.name || '' };
}

function buildTransitInstructions(route) {
  const steps = [];
  const seen = new Set();
  for (const leg of (route.legs || [])) {
    for (const step of (leg.steps || [])) {
      const info = getTransitStepSummary(step);
      if (!info) continue;
      const key = [info.start, info.end, info.lineName, info.headsign].join('|');
...
function normalizeRouteCachePart(value) {
  return String(value ?? '').trim().replace(/\s+/g, ' ').toLowerCase();
}

function buildGoogleRouteCacheKey(origin, destination, mode, departureTime) {
  return [
    'v1',
    normalizeRouteCachePart(origin),
    normalizeRouteCachePart(destination),
    normalizeRouteCachePart(mode),
    normalizeRouteCachePart(departureTime || '')
  ].join('||');
}

function readGoogleRouteCache(cacheKey) {
  try {
    const raw = localStorage.getItem(GOOGLE_ROUTES_CACHE_STORAGE);
    if (!raw) return null;
    const cache = JSON.parse(raw);
    const entry = cache?.[cacheKey];
    if (!entry || !entry.savedAt || !entry.route) return null;
    if (Date.now() - Number(entry.savedAt) > GOOGLE_ROUTES_CACHE_TTL_MS) {
      delete cache[cacheKey];
      localStorage.setItem(GOOGLE_ROUTES_CACHE_STORAGE, JSON.stringify(cache));
      return null;
    }
    return entry.route;
  } catch (e) {
...
function writeGoogleRouteCache(cacheKey, route) {
  try {
    const raw = localStorage.getItem(GOOGLE_ROUTES_CACHE_STORAGE);
    const cache = raw ? JSON.parse(raw) : {};
    cache[cacheKey] = { savedAt: Date.now(), route };

    // 避免長期累積太多資料：只保留最近 100 筆。
    const entries = Object.entries(cache)
      .sort((a, b) => Number(b[1]?.savedAt || 0) - Number(a[1]?.savedAt || 0))
      .slice(0, 100);
    localStorage.setItem(GOOGLE_ROUTES_CACHE_STORAGE, JSON.stringify(Object.fromEntries(entries)));
  } catch (e) {
    // 儲存空間不足時，不影響正常 API 查詢。
  }
}

function clearGoogleRoutesCache() {
  localStorage.removeItem(GOOGLE_ROUTES_CACHE_STORAGE);
}

async function fetchGoogleRoute(origin, destination, mode, departureTime) {
  const cacheKey = buildGoogleRouteCacheKey(origin, destination, mode, departureTime);

  // 第一次使用交通功能時，先確認已設定 Routes API Key。
  // 這樣即使瀏覽器裡已有舊快取，也會讓使用者知道如何設定 API Key；
  // 設定完成後，後續相同查詢仍會直接使用快取，不重複呼叫 API。
  const apiKey = await getGoogleRoutesApiKey();
  if (!apiKey) throw new Error('API Key 尚未設定（請在 config.js 填入 Google API Key）');

  const cachedRoute = readGoogleRouteCache(cacheKey);
  if (cachedRoute) {
    return { route: cachedRoute, fromCache: true };
  }

  // 同一時間已有相同查詢時，所有點擊共用同一個 Promise，不會重複呼叫 API。
  if (googleRoutesInFlight.has(cacheKey)) {
    return { route: await googleRoutesInFlight.get(cacheKey), fromCache: false, deduplicated: true };
  }

  const requestPromise = (async () => {
    let originWaypoint = { address: origin };
    let destinationWaypoint = { address: destination };
    try {
      const [originPlaceId, destinationPlaceId] = await Promise.all([
        resolvePlaceId(apiKey, origin),
        resolvePlaceId(apiKey, destination)
      ]);
      if (originPlaceId) originWaypoint = { placeId: originPlaceId };
      if (destinationPlaceId) destinationWaypoint = { placeId: destinationPlaceId };
    } catch (placeErr) {
      // Places API 是這版用來把景點名稱轉成 Place ID 的必要步驟；
      // 若未啟用，不再默默退回地址，直接把真正的設定問題告訴使用者。
      throw placeErr;
    }
  ...
  const fieldMask = [
    ...
    // 有些地點的地址解析或班次資料對指定出發時間較嚴格。
    // 若指定時間沒有任何方案，TRANSIT 再以「現在」重試一次，避免直接顯示找不到路線。
    if (!routes.length && mode === 'TRANSIT' && departureTime) {
      const fallbackBody = { ...body };
      delete fallbackBody.departureTime;
      const fallbackResponse = await fetch(GOOGLE_ROUTES_ENDPOINT, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Goog-Api-Key': apiKey,
      ...
      if (fallbackResponse.ok) {
        ...
        routes = Array.isArray(fallbackData.routes) ? fallbackData.routes : [];
      }
    }

    if (!routes.length) return null;
    routes.sort((a, b) => parseInt(a.duration || '0', 10) - parseInt(b.duration || '0', 10));
    const route = routes[0];
    writeGoogleRouteCache(cacheKey, route);
    return route;
  })();

  googleRoutesInFlight.set(cacheKey, requestPromise);
  try {
    const route = await requestPromise;
    return { route, fromCache: false, deduplicated: false };
  } finally {
    googleRoutesInFlight.delete(cacheKey);
  }
}

async function loadRouteEstimate(button) {
  if (!button) return;
  ...
  const spotTime = button.dataset.spotTime || '';
  if (!info) return;

  const modeMap = { transit: 'TRANSIT', driving: 'DRIVE', walking: 'WALK' };
  const labels = { transit: '🚆 大眾運輸', driving: '🚗 開車', walking: '🚶 步行' };
  const apiMode = modeMap[mode];
  const departureTime = buildDepartureTime(dayName, spotTime);
  ...
  // 保留原本點擊後直接開 Google Maps 的行為。
  window.open(mapsUrl, '_blank', 'noopener,noreferrer');
  info.style.display = 'block';
  info.innerHTML = `<strong>${labels[mode]}</strong>　正在取得${mode === 'transit' ? '最快路線與起訖站' : '預估時間'}…`;

  try {
    const result = await fetchGoogleRoute(origin, destination, apiMode, departureTime);
    const route = result?.route || null;
    if (!route) throw new Error('找不到可用路線');
    const duration = secondsToDurationText(parseInt(route.duration || '0', 10));
    const cacheNote = result?.fromCache ? '<span style=\"color:#2f855a;\">💾 已使用已儲存的路線結果</span><br>' : '';

    if (mode === 'transit') {
      const instructions = buildTransitInstructions(route);
      const fare = formatTransitFare(route.travelAdvisory?.transitFare);
      if (instructions.length) {
        // 顯示最快方案中的每一段交通工具與其起訖站。
        const lines = instructions.map(x => {
          const vehicle = [x.lineName, x.headsign ? `往 ${x.headsign}` : '', x.vehicle ? `（${x.vehicle}）` : ''].filter(Boolean).join(' ');
          return `<div style="margin-top:5px;"><strong>${escapeHtml(x.start || '起點站')} → ${escapeHtml(x.end || '終點站')}</strong>${vehicle ? `<br><span style="color:#52606d;">${escapeHtml(vehicle)}</span>` : ''}</div>`;
        }).join('');
        info.innerHTML = `${cacheNote}<strong>🚆 大眾運輸</strong>　最快約 <strong>${escapeHtml(duration)}</strong><br>${lines}<br><span style="color:#52606d;">💴 ${escapeHtml(fare)}</span>`;
      } else {
        info.innerHTML = `${cacheNote}<strong>🚆 大眾運輸</strong>　最快約 <strong>${escapeHtml(duration)}</strong><br><span style="color:#777;">未取得詳細起訖站資料，請查看 Google Maps。</span>`;
      }
    } else {
      info.innerHTML = `${cacheNote}<strong>${labels[mode]}</strong>　預估時間：<strong>${escapeHtml(duration)}</strong><br><span style="color:#52606d;">${escapeHtml(origin)} → ${escapeHtml(destination)}</span>`;
    }
  } catch (err) {
    const msg = String(err?.message || '請查看 Google Maps');
    let extra = '';
    if (msg.includes('API Key 尚未設定') || msg.includes(' 401') || msg.includes(' 403') || msg.includes('REQUEST_DENIED') || msg.includes('PERMISSION_DENIED') || msg.includes('API key')) {
      extra = '<br><span style="color:#a33;">請確認同目錄的 config.js 已填入 Google API Key，且該 Key 已限制為 Routes API + Places API。</span>';
    }
    if (msg.includes('Places API 403') || msg.includes('REQUEST_DENIED') || msg.includes('PERMISSION_DENIED')) {
      extra += '<br><span style="color:#a33;">請在 Google Cloud 啟用「Places API (New)」，並把這把 API Key 的 API 限制加入 Places API。</span>';
    }
    info.innerHTML = `<strong>${labels[mode]}</strong>　無法取得資料<br><span style="color:#777;">${escapeHtml(msg)}</span>${extra}`;
  }
}
...
function renderViewPage() {
  ...
  if (!trip) {
    // 直接開啟 view.html 或使用失效 id 時，靜默回首頁，不再跳出錯誤視窗。
...
// --- 3. 詳細頁 (day.html) ---
function selectRouteMode(buttonId, infoId, mode, origin, destination) {
  const buttons = document.querySelectorAll(`[id^="routeBtn_${infoId.replace('routeInfo_', '')}_"]`);
  buttons.forEach(btn => {
    btn.style.background = '#fff';
    btn.style.color = '#31506b';
    btn.style.borderColor = '#d9e2ec';
    btn.style.boxShadow = 'none';
    btn.style.transform = 'none';
  });
  const selected = document.getElementById(buttonId);
  if (selected) {
    selected.style.background = '#31506b';
    selected.style.color = '#fff';
    selected.style.borderColor = '#31506b';
    selected.style.boxShadow = '0 2px 5px rgba(49,80,107,.22)';
    selected.style.transform = 'translateY(1px)';
  }
  if (selected) loadRouteEstimate(selected);
}
...
function renderDayPage() {
  ...
  if (!trip || !trip.days || !trip.days[dayIndex]) {
    // 直接開啟 day.html 或使用失效 id/day 時，靜默回首頁。
    window.location.replace('index.html');
    return;
  }
  ...
  dayData.spots.forEach((spot, idx) => {
    ...
    if (idx < dayData.spots.length - 1) {
      ...
      const dayNameForRoute = dayData.name || '';
      const routeModes = [
      ...
      const routeButtons = routeModes.map(({ mode, label }) => {
        const buttonId = `routeBtn_${dayIndex}_${idx}_${mode}`;
        const infoId = `routeInfo_${dayIndex}_${idx}`;
        return `<button id="${buttonId}" type="button" onclick="selectRouteMode('${buttonId}','${infoId}','${mode}')" data-mode="${mode}" data-origin="${escapeHtml(cleanName)}" data-destination="${escapeHtml(nextName)}" data-day-name="${escapeHtml(dayNameForRoute)}" data-spot-time="${escapeHtml(spot.time || '')}" data-info-id="${infoId}" style="display:inline-flex;align-items:center;justify-content:center;background:#fff;border:1px solid #d9e2ec;color:#31506b;border-radius:8px;padding:7px 9px;font-size:12px;font-weight:700;cursor:pointer;transition:all .15s ease;">${label}</button>`;
      }).join('');
      transitHtml = `
        <div class=\"transit-box\" style=\"margin:6px 0 12px 0;padding:10px 12px;background:#f7f9fc;border:1px solid #e5eaf0;border-radius:10px;\">
          ...
          <div id=\"routeInfo_${dayIndex}_${idx}\" style=\"display:none;margin-top:8px;padding:7px 9px;background:#fff;border:1px solid #d9e2ec;border-radius:8px;font-size:12px;color:#52606d;line-height:1.5;\"></div>
        </div>
      `;
    }
